			 ...
			 loff_t *ppos)
{
	...
	return lefuse_direct_io(file, buf, count, &pos, 0);
}
			  ...
			  size_t count, loff_t *ppos)
{
	 ...
	 */
	return lefuse_direct_io(file, buf, count, &pos, 1);
}
...
static int cuse_open(struct inode *inode, struct file *file)
{
	 ...
	 */
	rc = lefuse_do_open(&cc->fc, 0, file, 0);
	if (rc)
		lefuse_conn_put(&cc->fc);
	return rc;
}
...
static int cuse_release(struct inode *inode, struct file *file)
{
	...
	lefuse_sync_release(ff, file->f_flags);
	lefuse_conn_put(fc);
...
static int cuse_channel_release(struct inode *inode, struct file *file)
{
	...
	lefuse_conn_kill(&cc->fc);
	rc = lefuse_dev_release(inode, file);	/* puts the base reference */
...
static int __init cuse_init(void)
{
	...
	/* inherit and extend lefuse_dev_operations */
	cuse_channel_fops		= lefuse_dev_operations;
